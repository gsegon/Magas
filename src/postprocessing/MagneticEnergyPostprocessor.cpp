...
#include "include/MagneticEnergyPostprocessor.h"

using namespace dealii;

template class MagneticEnergyPostprocessor<2>;


template <int dim>
MagneticEnergyPostprocessor<dim>::MagneticEnergyPostprocessor(const std::unordered_map<int, double>& nu_map) {
    this->nu_map_ptr = &nu_map;
}

template<int dim>
void MagneticEnergyPostprocessor<dim>::process(const Triangulation<dim>&  triangulation,
    ...
    for (auto cell : dof_handler.active_cell_iterators()){
        ...
        for (const unsigned int q: fe_values.quadrature_point_indices())
        {
            ...
            E_cell += (b_x*b_x+b_y*b_y)/2.0 * nu_cell* fe_values.JxW(q);
