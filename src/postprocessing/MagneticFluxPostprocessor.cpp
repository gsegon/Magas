...
#include "include/MagneticFluxPostprocessor.h"
...
template <int dim>
MagneticFluxPostprocessor<dim>::MagneticFluxPostprocessor(unsigned int quadrature_point) {
    this->q_index = quadrature_point;
    this->abs = true;

}

template <int dim>
MagneticFluxPostprocessor<dim>::MagneticFluxPostprocessor(unsigned int quadrature_point, unsigned int component) {
    this->q_index = quadrature_point;
    this->abs = false;
...
void MagneticFluxPostprocessor<dim>::process(const Triangulation<dim>&  triangulation,
    ...
    for (auto cell : dof_handler.active_cell_iterators()){
        ...
        if (this->abs) {
            result.push_back(std::sqrt(std::pow(solution_gradients[q_index][1], 2) + std::pow(-solution_gradients[q_index][0], 2)));
        }
        else{
            if (component == 0){
                result.push_back(solution_gradients[q_index][1]);
            }
            else if (component == 1){
                result.push_back(-solution_gradients[q_index][0]);
            }
        }
